...
import ImageChops, sys, ImageDraw, serial, time, bisect, cv2, scipy, threading
...
def serialOut(dat, Dist1, Dist2, fireDict, t2, lock):
...
##            Dist2 = ord(Dist2)
        ...
        if not np.isnan(xx) and not np.isnan(yy):   # There is a small chance there is no fire in the frame
            ...
            dat = xzone, yzone
            Read = serialOut(dat, Dist1, Dist2, fireDict, t2, lock) #Assumes Cameras are Upside Down
            if (Read != ""):
...
try:
    ...
    ser = serial.Serial('COM10',baudrate = 115200, timeout = .02)#Inizalize Serial Port. The 0 is the port number. This will may if you change computers.
    ...
    ir0 = PIL.Image.open("irFirstImage.bmp")
        ...
        ##while True:
    lock = threading.Lock()
    t1 = threading.Thread(target = serialQuery,args = (lock,))
    t1.daemon = True
    t1.start()
    t2 = threading.Thread(target = MainLoop, args = (ir0,lock,))
    t2.daemon = True
    t2.start()
    while True:
        if not t2.isAlive():
