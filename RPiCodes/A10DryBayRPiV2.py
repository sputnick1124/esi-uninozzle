...
def getImage(cam,imstream):
    imstream.seek(0)
    imstream.truncate()
    cam.capture(imstream, format = 'jpeg', use_video_port = True)
    imstream.seek(0)
    return PIL.Image.open(imstream)
...
def MainLoop(ir0,cam):
    ...
    imstream1 = io.BytesIO()
    imstream2 = io.BytesIO()
    while True:
##            ser.write(chr(254))
##            ser.write(chr(150))
##            Dist1 = ser.read(10)
...
##            Dist2 = ser.read(10)
        ...
        while not ser.inWaiting():
            pass
        Gate1 = ser.read(10) # Read "Gate" pin
##            Dist1 = ord(Dist1)
##            Dist2 = ord(Dist2)
        Dist1 = 0
        Dist2 = 0
        ...
        if not Gate < 120: # If "Gate" is not open
            ...
            ir1 = getImage(cam,imstream1)   # Always take a new picture for reference
            ser.write(chr(254))
            ser.write(chr(153))
            while not ser.inWaiting():
                pass
            Override1 = ser.read(10)
            Override = ord(Override1)
            ser.flushInput()
            ser.flushInput()
            ser.flushOutput()
            ser.flush()
            if Override < 120:      # If "Override" has been triggered
        ...
        t2 = time.clock()            
        ir2 = getImage(cam,imstream2)   # Take a picture of the fire
...
try:
    ...
    #               ------------------------------------------
        ...
        ser = serial.Serial('/dev/ttyUSB0',baudrate = 115200,timeout = .02) #Inizalize Serial Port. The 0 is the port number. This will may if you change computers.
...
##        cam.contrast = con
        ...
        while ( rgbavg > 12) or (gmax > 150):
            ...
            cam.capture('irCalib.bmp',use_video_port = True)
        ...
        cam.capture('irFirstImage.bmp',use_video_port = True)
